...
摄影评审 Agent：我将自己的摄影经验蒸馏，形成了严密的判断标准，经过几次测试下来基本符合预期，能点出摄影作品需要改进的地方，前期建议和后期建议均有。
流程：图片进来 → 判定是否 AI 生成 →（AI）AI 板块评审 + 提示词改进建议；（非 AI）严格分类风景/人物（其他拒绝）→ 按板块 rubric 评分 + 给出改进方案（前期拍摄 / 后期修图分组）。
...
- **自动压缩**：图片超过 `PHOTO_AGENT_MAX_IMAGE_MB`（默认 10MB）时自动压缩到长边 2000px 内；超过 5 倍上限（默认 50MB）返回 413。元数据检测完成后，送 LLM 评审的图片统一压到长边 ≤2000px、≤1MB 以内（压缩在内存中完成，原图不会被修改）。
